...
void AWarriorProjectileBase::OnProjectileHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (bIsValidBlock)
	{
		UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(HitPawn, WarriorGameplayTags::Player_Event_SuccessfullBlock, Data);
	}
	else
	{
		HandleApplyProjectileDaamge(HitPawn, Data);
	}
...
void AWarriorProjectileBase::HandleApplyProjectileDaamge(APawn* InHitPawn, const FGameplayEventData& InPayload)
{
	checkf(ProjectileDamageEffectSpecHandle.IsValid(), TEXT("Forgot to assign valid spec handle to the projectile: %s"), *GetActorNameOrLabel());
